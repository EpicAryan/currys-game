...
export async function getCurrentCampaignDay(
  campaignStartDate: Date
): Promise<number> {
  const now = new Date();
  const startDay = startOfDay(campaignStartDate);
...
export async function isWithinCampaignPeriod(
  currentDate: Date,
  startDate: Date,
  endDate: Date
): Promise<boolean> {
...
export async function getGiftAvailabilityWindow(
  dayNumber: number,
  campaignStartDate: Date
): Promise<GiftAvailabilityWindow> {
  if (dayNumber < 1 || dayNumber > 12) {
    throw new Error("Day number must be between 1 and 12");
  }
...
export async function isGiftAvailable(
  dayNumber: number,
  campaignStartDate: Date,
  currentDate: Date = new Date()
): Promise<boolean> {
  const { availableFrom, availableUntil } =
    await getGiftAvailabilityWindow(dayNumber, campaignStartDate);
...
export async function isGiftMissed(
  dayNumber: number,
  campaignStartDate: Date,
  currentDate: Date = new Date()
): Promise<boolean> {
